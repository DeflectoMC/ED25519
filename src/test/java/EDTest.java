...
public class EDTest {
	public static void main(String[] args) {
		...
		byte[] sk = hash256("This is my secret key, Tell NOBODY! It's a secret.");
		System.out.println("sk: "+Base64v2.encode(sk) + " (" + (sk.length *8) + " bits)");
		timer.reset();
		byte[] pk = ED25519.publicKey(sk);
		System.out.println("pk: "+Base64v2.encode(pk)+" ("+timer.lap()+" ms)");
		byte[] m = "This is my message. You can tell people about this one.".getBytes();
		...
		for(int c=0;c<10;c++) {
			...
			timer.reset();
			sig = ED25519.sign(m, sk, pk);
			System.out.println("sign: " + Base64v2.encode(sig) + " (" + timer.lap() + " ms)");

			// test verifying signature
			timer.reset();
			boolean isGood = ED25519.checkValid(sig, m, pk);
			if(isGood) System.out.println("Signature is good ("+timer.lap()+" ms)");
			else System.out.println("Signature is NOT good ("+timer.lap()+" ms)");

			System.out.println();
		}

		// *** notice the code runs faster as the JIT compiler does its work ***

		// Bx.mod(q),By.mod(q)
//		System.out.println(ED25519.Bx.mod(ED25519.q));
//		System.out.println(ED25519.By.mod(ED25519.q));
	}

	static public byte[] hash256(String m) {
		MessageDigest md;
		try {
			md = MessageDigest.getInstance("SHA-256");
			md.reset();
			return md.digest(m.getBytes());
	...
	############################################################
	Moved helper classes inside test class to prevent confusion.
	Only the ED25519 class is needed to sign and verify messages
	############################################################

	 */

	public static class Base64v2 {
		/**
